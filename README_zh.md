...
Agora 是一个开源桌面 AI Agent 控制台。它可以安装或复用 OpenCode（默认引擎）、Claude Code、Codex、Qwen Code、DeepSeek-TUI 和内置 Agent runtime，把它们统一到一个可视化工作台里，覆盖对话、工具、文件、IM 通道、技能、模型供应商、运行监控和桌面宠物工作流。

> 早期公开版本优先提供 macOS Apple Silicon 安装包。如果 Agora 对你的 Agent 工作流有帮助，欢迎点亮 Star，让更多开发者看到这个项目。

## 快速入口

- 官网：[agora.app](https://agora.app/)
- 最新版本：[github.com/freestylefly/agora/releases/latest](https://github.com/freestylefly/agora/releases/latest)
- 产品截图：[产品截图](#产品截图)
- 核心功能：[核心功能](#核心功能)
...
终端原生编码 Agent 很强，但安装、模型路由、权限、IM 入口、文件变更和运行指标经常散落在不同位置。Agora 把这些环节收进一个桌面工作台：

- 用新手友好的界面安装、检测和复用本机 Agent CLI。
- 通过可视化 Chat 使用编码 Agent，直接查看工具面板、Slash 指令、文件 diff 和权限事件。
- 将 Agent 任务接入 7 个 IM 平台：飞书、钉钉、Telegram、Discord、企业微信、网易云信 IM、微信公众号。
- 记录每次任务调用的引擎、模型、Token、TTFT、TPS、工具耗时、步骤、状态和总耗时。
- 通过 SkillHub 技能、内置技能、定时任务、记忆系统和桌面宠物扩展日常工作流。

## 核心功能

1. **Agent 引擎中心**：统一运行 OpenCode（默认引擎）、Claude Code、Codex、Qwen Code、DeepSeek-TUI 和内置 runtime，支持一键安装或复用本机 CLI 配置。
...
3. **IM Agent Hub**：将飞书、钉钉、Telegram、Discord、企业微信、网易云信 IM、微信公众号的消息接入任意引擎，支持按平台配置机器人。
4. **AI Runtime Dashboard**：按引擎、模型、来源、状态、Token、总耗时、TTFT、输出阶段 TPS、估算 Model TPS、工具耗时和 Agent 步数统计调用。
5. **SkillHub 技能市场**：发现、分类、安装、启用、禁用、更新和删除本机技能。
6. **定时任务与记忆**：创建研究报告、监控、邮箱整理、提醒等重复任务，自动提取对话记忆并跨会话延续上下文。
...
<table>
  ...
  <tr>
    ...
    <td><strong>Agent 引擎</strong><br>配置 OpenCode、Claude Code、Codex、Qwen Code、DeepSeek-TUI 和内置 runtime。</td>
  </tr>
  <tr>
    <td width="50%">
  ...
  <tr>
    ...
    <td width="50%">
      <img src="public/readme/screenshots/studio-pet.png" alt="Agora 工作室和桌面宠物">
    </td>
  </tr>
  <tr>
    <td><strong>技能市场</strong><br>按 SkillHub 分类浏览技能，下载安装到本机 Agora 技能目录。</td>
...
| 内置 runtime | 通用桌面 Cowork 会话和技能执行                  | Agora 内置                                   |

## IM 平台

Agora 通过 7 个 IM 平台路由 Agent 任务，支持按平台配置机器人：
...
Agora 把模型配置集中在一个设置页。某个引擎选择"跟随 Agora 模型设置"时，Agora 会把模型配置映射到该引擎。
...
从 [最新版本](https://github.com/freestylefly/agora/releases/latest) 下载 `Agora-*-arm64.dmg`，打开后将 `Agora.app` 拖入 `Applications` 文件夹。

<p align="center">
  <img src="public/readme/tutorial/install-dmg.svg" alt="Agora DMG 安装示意图" width="760">
</p>
...
命令执行完成后，重新打开 Agora 即可。如果你没有把 Agora 放到 `Applications` 文件夹，请把命令里的路径换成实际的 `Agora.app` 路径。
...
```bash
# 启动 Agora，并在设置中检测支持的本机 Agent CLI
npm run electron:dev
...
Agora 使用 Electron 进程隔离架构。Renderer 不直接访问 Node.js 能力，高权限操作通过 preload bridge 和 main process IPC 完成。

<p align="center">
  <img src="public/readme-architecture.svg" alt="Agora 架构原理图" width="960">
</p>
...
Agora 内置了一组覆盖日常 Agent 工作的技能，并接入 SkillHub 用于技能市场安装。
...
欢迎扫描下方二维码加入 Agora 微信交流群，和其他使用者、开发者一起交流。二维码有效期至 2026 年 6 月 8 日；如果过期，可以通过下方公众号获取最新入群方式。

<p align="center">
  <img src="public/readme/community/wechat-group.jpg" alt="Agora 微信交流群" width="320">
</p>
...
Agora 的设计与工程实现受到了许多优秀开源项目和 Agent 社区实践的启发，特别感谢：
