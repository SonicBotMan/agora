...
test('parseManagedSessionKey handles raw local session keys', () => {
  assert.deepEqual(parseManagedSessionKey('agora:abc-123'), {
    agentId: null,
    sessionId: 'abc-123',
  });
  assert.deepEqual(parseManagedSessionKey('agora:abc-123'), {
...
test('parseManagedSessionKey handles canonical local session keys', () => {
  assert.deepEqual(parseManagedSessionKey('agent:main:agora:abc-123'), {
    agentId: 'main',
    sessionId: 'abc-123',
  });
  assert.deepEqual(parseManagedSessionKey('agent:main:agora:abc-123'), {
...
test('buildManagedSessionKey emits canonical local session keys', () => {
  assert.equal(
    ...
    `agent:${DEFAULT_MANAGED_AGENT_ID}:agora:abc-123`,
  );
  assert.equal(
    buildManagedSessionKey('abc-123', 'secondary'),
    'agent:secondary:agora:abc-123',
  );
});

test('parseChannelSessionKey ignores managed local session keys', () => {
  assert.equal(parseChannelSessionKey('agora:abc-123'), null);
  assert.equal(parseChannelSessionKey('agent:main:agora:abc-123'), null);
  assert.equal(parseChannelSessionKey('agora:abc-123'), null);
  assert.equal(parseChannelSessionKey('agent:main:agora:abc-123'), null);
});
...
test('channel sync does not treat managed local session keys as channel sessions', () => {
  ...
  assert.equal(isManagedSessionKey('agent:main:agora:abc-123'), true);
  assert.equal(sync.isChannelSessionKey('agent:main:agora:abc-123'), false);
  assert.equal(sync.resolveOrCreateSession('agent:main:agora:abc-123'), null);
  assert.equal(sync.resolveOrCreateMainAgentSession('agent:main:agora:abc-123'), null);
});
