...
const createSessionStore = () => ({
  'agent:main:agora:current-session': {
  ...
  'agent:main:agora:old-claude-session': {
...
test('sync writes native moonshot provider config and migrates matching managed sessions', (t) => {
  ...
  assert.equal(sessionStore['agent:main:agora:current-session'].modelProvider, 'moonshot');
  assert.equal(sessionStore['agent:main:agora:current-session'].model, 'kimi-k2.5');
  assert.equal(sessionStore['agent:main:agora:current-session'].systemPromptReport.provider, 'moonshot');
  assert.equal(sessionStore['agent:main:agora:old-claude-session'].modelProvider, 'lobster');
  assert.equal(sessionStore['agent:main:agora:old-claude-session'].model, 'claude-sonnet-4-5-20250929');
...
test('sync maps moonshot coding plan sessions to kimi-coding model refs', (t) => {
  ...
  assert.equal(sessionStore['agent:main:agora:current-session'].model, 'kimi-k2.5');
  assert.equal(sessionStore['agent:main:agora:current-session'].systemPromptReport.provider, 'moonshot');
  assert.equal(sessionStore['agent:main:agora:current-session'].systemPromptReport.model, 'kimi-k2.5');
  assert.equal(sessionStore['agent:main:wecom:direct:wangning'].execSecurity, 'full');
  assert.equal(sessionStore['agent:main:feishu:dm:ou_123'].execSecurity, 'full');
  assert.equal('skillsSnapshot' in sessionStore['agent:main:wecom:direct:wangning'], false);
  assert.equal('skillsSnapshot' in sessionStore['agent:main:feishu:dm:ou_123'], false);
...
test('sync denies exec for native channel sessions even without provider migration', (t) => {
  ...
  const sessionStore = JSON.parse(fs.readFileSync(path.join(sessionsDir, 'sessions.json'), 'utf8'));
  assert.equal(sessionStore['agent:main:agora:current-session'].modelProvider, 'openai');
  assert.equal(sessionStore['agent:main:agora:current-session'].model, 'kimi-k2.5');
  assert.equal(sessionStore['agent:main:wecom:direct:wangning'].execSecurity, 'full');
  assert.equal(sessionStore['agent:main:feishu:dm:ou_123'].execSecurity, 'full');
  assert.equal('skillsSnapshot' in sessionStore['agent:main:wecom:direct:wangning'], false);
...
test('sync preserves existing AGENTS.md content above the Agora managed marker', (t) => {
  ...
  assert.match(agentsMd, /<!-- Agora managed: do not edit below this line -->/);
...
test('sync writes non-empty placeholder apiKey for providers that do not require auth (e.g. Ollama)', (t) => {
  ...
  assert.equal(providerConfig.apiKey, '${AGORA_APIKEY_OLLAMA}');
});
