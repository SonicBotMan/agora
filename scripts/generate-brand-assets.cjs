...
function generateMacIcon() {
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'agora-mac-icon-'));
...
function createIco(source, outputPath, sizes) {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'agora-ico-'));
  try {
...
function generateBanner() {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'agora-banner-'));
  try {
  ...
  <text x="404" y="156" fill="#FFF7DF" font-family="Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" font-size="72" font-weight="800" letter-spacing="0">Agora</text>
...
function main() {
  ...
  if (macOnly) {
    ...
    generateMacIcon();
    console.log(`Generated Agora macOS icon from ${logoPath}`);
    return;
  }
  copySourceLogo();
  generatePngIcons();
  ...
  console.log(`Generated Agora brand assets from ${sourcePath}`);
}
