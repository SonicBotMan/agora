...
Eliminate OpenClaw gateway process restarts when users switch between 套餐模型 (agora-server) and 自定义模型 (custom providers), or between different custom providers with different apiKeys.
...
resolveAllProviderApiKeys() ──────────────────────────┐
  (claudeSettings.ts)                                  │
  Enumerates all enabled providers + agora-server   │
...
|----------|-------------|--------|
| agora-server | `LOBSTER_APIKEY_SERVER` | accessToken from auth |
| moonshot | `LOBSTER_APIKEY_MOONSHOT` | provider config apiKey |
| anthropic | `LOBSTER_APIKEY_ANTHROPIC` | provider config apiKey |
| ollama | `LOBSTER_APIKEY_OLLAMA` | `sk-agora-local` (no key needed) |
...
For agora-server, hardcoded as `SERVER` (since it's a dynamic provider, not in app_config.providers).
...
2. `buildProviderSelection()` — all 4 cases updated:
   - agora-server: `${LOBSTER_APIKEY_SERVER}` (was inline apiKey)
