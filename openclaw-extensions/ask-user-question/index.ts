 ...
 * and waits for the user's response via an HTTP callback to Agora.
 *
 * This enables delete-confirmation modals on the Agora desktop app
...
const plugin = {
  ...
  description: 'Structured user confirmation tool for Agora desktop.',
  configSchema: {
  ...
  register(api: OpenClawPluginApi) {
    ...
    api.registerTool((ctx) => {
      // Only enable for Agora desktop sessions. Legacy session keys remain supported.
      ...
      const isLocalDesktop = sessionKey.startsWith('agent:main:agora:')
        || sessionKey.startsWith('agent:main:agora:');
      if (!isLocalDesktop) {
        return null;
      }
