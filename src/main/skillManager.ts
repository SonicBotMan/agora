...
const downloadGithubArchive = async (
    ...
    archiveUrlCandidates.push(
      {
        ...
        headers: { 'User-Agent': 'Agora Skill Downloader' },
      },
      {
  ...
  archiveUrlCandidates.push({
    ...
    headers: {
      ...
      'User-Agent': 'Agora Skill Downloader',
...
export class SkillManager {
      ...
      if (fs.existsSync(localSource)) {
        ...
        if (stat.isFile()) {
          if (isZipFile(localSource)) {
            ...
            const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'agora-skill-zip-'));
        ...
        const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'agora-skill-npm-'));
        cleanupPath = tempRoot;
        ...
        const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'agora-skill-clawhub-'));
        cleanupPath = tempRoot;
        ...
        const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'agora-skill-skillhub-'));
        cleanupPath = tempRoot;
        const env = buildSkillEnv();
        ...
        const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'agora-skill-'));
        cleanupPath = tempRoot;
  ...
  private async performUpgradeDownload(skillId: string, downloadUrl: string, existingSkillDir: string): Promise<{
    ...
    try {
      ...
      const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'agora-skill-upgrade-'));
      cleanupPath = tempRoot;
