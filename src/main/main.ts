...
// (registered via ./ipc/permissionHandlers)
  ...
  const hasEnabledOpenClawManagedIMPlatform = (): boolean => {
    ...
    const feishuManagedByAgora = isFeishuEngineManagedByAgora(FeishuEngineKey.OpenClaw);
    ...
    return Boolean(
      ...
      || (feishuManagedByOpenClaw && feishuManagedByAgora && config.feishu?.instances?.some(i => i.enabled && i.appId && i.appSecret))
  ...
  ipcMain.handle(ImIpcChannel.FeishuSetRuntimeOwnership, async (_event, input: unknown) => {
    try {
      ...
      const transferResult = ownership === FeishuRuntimeOwnership.LocalRuntime
        ...
        : await transferFeishuToAgoraRuntime(engineKey);
