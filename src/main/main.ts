...
import { registerImHandlers } from './ipc/imHandlers';
...
const isFeishuEngineManagedByAgora = (engineKey: FeishuEngineKeyType): boolean => (
...
const syncOpenClawConfig = async (
  ...
  const shouldUseManagedGateway = getCoworkStore().getConfig().openclawConfigSource === ExternalAgentConfigSource.AgoraModel;
...
const bindCoworkRuntimeForwarder = (): void => {
  ...
  runtime.on('complete', (sessionId: string, claudeSessionId: string | null) => {
    ...
    try {
      ...
      if (apiConfig.providerMetadata?.providerName === 'agora-server') {
        const windows = BrowserWindow.getAllWindows();
        windows.forEach((win) => {
          if (win.isDestroyed()) return;
...
if (!gotTheLock) {
  ...
  /**
   * Parse a agora:// deep link and send (or buffer) the auth code.
   */
  ...
  ipcMain.handle('cowork:config:set', async (_event, config: {
    ...
    try {
      ...
      if (switchedAwayFromHermes) {
        ...
        if (isFeishuEngineManagedByAgora(FeishuEngineKey.Hermes)) {
...
// (registered via ./ipc/permissionHandlers)
  // IM Gateway + IM instances + Feishu + DingTalk + QQ + Weixin + pairing handlers
  // (registered via ./ipc/imHandlers)
  registerImHandlers({
    getStore: () => getStore(),
    getCoworkStore: () => getCoworkStore(),
    getIMGatewayManager: () => getIMGatewayManager() as any,
    getOpenClawEngineManager: () => getOpenClawEngineManager(),
    getHermesEngineManager: () => getHermesEngineManager(),
    getOpenClawConfigSync: () => getOpenClawConfigSync(),
    getHermesConfigSync: () => getHermesConfigSync(),
    openClawRuntimeAdapter: openClawRuntimeAdapter,
    syncOpenClawConfig: (options) => syncOpenClawConfig(options),
    resolveFeishuIMAgentEngine: () => resolveFeishuIMAgentEngine(),
    isFeishuEngineManagedByAgora: (k) => isFeishuEngineManagedByAgora(k as any),
    normalizeFeishuEngineKey: (v) => normalizeFeishuEngineKey(v as any),
    getFeishuRuntimeOwnership: (k) => getFeishuRuntimeOwnership(k as any),
    getFeishuRuntimeOwnershipStatus: (k, o) => getFeishuRuntimeOwnershipStatus(k as any, o as any),
    transferFeishuToLocalRuntime: (k, i, m) => transferFeishuToLocalRuntime(k as any, i as any, m as any),
    transferFeishuToAgoraRuntime: (k) => transferFeishuToAgoraRuntime(k as any),
    detectLocalOpenClawFeishu: () => detectLocalOpenClawFeishu() as any,
    importOpenClawLocalFeishuConfig: () => importOpenClawLocalFeishuConfig() as any,
    listPairingRequests: (p, s) => listPairingRequests(p, s) as any,
    readAllowFromStore: (p, s) => readAllowFromStore(p, s),
    approvePairingCode: (p, c, s) => approvePairingCode(p, c, s) as any,
    rejectPairingRequest: (p, c, s) => rejectPairingRequest(p, c, s) as any,
    startHermesIMSessionSyncPolling: () => startHermesIMSessionSyncPolling(),
    syncHermesIMSessionsToCowork: (r) => syncHermesIMSessionsToCowork(r),
  });

  // GitHub Copilot device code authentication handlers
  // CORS-bypass HTTP proxy + GitHub Copilot device flow + persisted API config
  // (registered via ./ipc/apiHandlers)
  registerApiHandlers({
    getStore: () => getStore(),
    getCoworkStore: () => getCoworkStore(),
    getHermesConfigSync: () => getHermesConfigSync(),
  ...
  const initApp = async () => {
    ...
    const defaultProjectDir = path.join(os.homedir(), 'agora', 'project');
    ...
    // Inject auth getters for agora-server provider routing
    ...
    registerProxyTokenRefresher('agora-server', async () => {
      const tokens = getAuthTokens();
      if (!tokens?.refreshToken) return null;
      const serverBaseUrl = getServerApiBaseUrl();
      try {
    ...
    const coldStartDeepLink = process.argv.find(arg => arg.startsWith('agora://'));
