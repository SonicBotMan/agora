...
const buildInstallScript = (target: InstallTarget): string => {
  ...
  if (method.id === 'official-installer') {
    ...
    return [
      ...
      `echo "__AGORA_INSTALL_METHOD__=${method.id}"`,
      installCommand,
      ...
      '  exit 127',
      'fi',
      'echo "__AGORA_BINARY_PATH__=${BINARY_PATH}"',
      'VERSION_OUTPUT=$({ "$BINARY_PATH" --version 2>&1 || true; } | head -n 1)',
      'echo "__AGORA_VERSION__=${VERSION_OUTPUT}"',
    ].join('\n');
  }
  ...
  return [
    ...
    '    echo "__AGORA_INSTALL_METHOD__=homebrew-bootstrap"',
    '    NONINTERACTIVE=1 /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"',
    '    if [ -x /opt/homebrew/bin/brew ]; then eval "$(/opt/homebrew/bin/brew shellenv)"; fi',
    '    if [ -x /usr/local/bin/brew ]; then eval "$(/usr/local/bin/brew shellenv)"; fi',
    '  fi',
    '  if ! command -v npm >/dev/null 2>&1; then',
    '    echo "__AGORA_INSTALL_METHOD__=homebrew-node"',
    '    brew install node',
    '  fi',
    'fi',
    'if ! command -v npm >/dev/null 2>&1; then',
    ...
    return new Promise<ExternalAgentCliInstallResult>((resolve) => {
      ...
      const handleOutput = (chunk: Buffer, source: 'stdout' | 'stderr') => {
        ...
        for (const rawLine of text.split(/\r?\n/)) {
          ...
          if (!line) continue;
          if (line.startsWith('__AGORA_INSTALL_METHOD__=')) {
            installMethod = line.slice('__AGORA_INSTALL_METHOD__='.length).trim() || undefined;
            this.emitProgress({
              appType,
              phase: 'installing',
          ...
          if (line.startsWith('__AGORA_BINARY_PATH__=')) {
            binaryPath = line.slice('__AGORA_BINARY_PATH__='.length).trim() || null;
            this.emitProgress({
              appType,
              phase: 'verifying',
          ...
          if (line.startsWith('__AGORA_VERSION__=')) {
            version = line.slice('__AGORA_VERSION__='.length).trim() || null;
            continue;
          }
          this.emitProgress({
            appType,
