...
describe('providerApiKeyEnvVar', () => {
  test('converts simple provider names', () => {
    expect(providerApiKeyEnvVar(ProviderName.Moonshot)).toBe('AGORA_APIKEY_MOONSHOT');
    expect(providerApiKeyEnvVar(ProviderName.Anthropic)).toBe('AGORA_APIKEY_ANTHROPIC');
    expect(providerApiKeyEnvVar(ProviderName.OpenAI)).toBe('AGORA_APIKEY_OPENAI');
    expect(providerApiKeyEnvVar(ProviderName.Ollama)).toBe('AGORA_APIKEY_OLLAMA');
  });

  test('replaces hyphens and special chars with underscores', () => {
    expect(providerApiKeyEnvVar(ProviderName.AgoraServer)).toBe('AGORA_APIKEY_AGORA_SERVER');
    expect(providerApiKeyEnvVar('my.provider')).toBe('AGORA_APIKEY_MY_PROVIDER');
  });

  test('server key matches hardcoded convention', () => {
    expect(providerApiKeyEnvVar('server')).toBe('AGORA_APIKEY_SERVER');
  });
});
...
describe('env var stability on model switch', () => {
  const simulateCollectEnvVars = (providers: Record<string, { enabled: boolean; apiKey: string }>, serverToken?: string) => {
    ...
    if (serverToken) {
      env.AGORA_APIKEY_SERVER = serverToken;
    }

    for (const [name, config] of Object.entries(providers)) {
      if (!config.enabled) continue;
      const envName = name.toUpperCase().replace(/[^A-Z0-9]/g, '_');
      env[`AGORA_APIKEY_${envName}`] = config.apiKey;
    }
  ...
  test('switching between two custom providers does not change env var keys', () => {
    ...
    expect(JSON.stringify(envBefore)).toBe(JSON.stringify(envAfter));
    expect(envBefore.AGORA_APIKEY_MOONSHOT).toBe('sk-moon-123');
    expect(envBefore.AGORA_APIKEY_ANTHROPIC).toBe('sk-ant-456');
  });
...
describe('resolveDescriptor', () => {
  ...
  test('unknown provider keeps its providerId', () => {
    const d = resolveDescriptor('some-unknown', false);
    expect(d.providerId).toBe('some-unknown');
  });

  test('empty provider name falls back to Agora', () => {
    const d = resolveDescriptor('', false);
    expect(d.providerId).toBe(OpenClawProviderId.Agora);
  });
...
describe('provider registry coverage', () => {
  ...
  test('no provider resolves to the Agora fallback', () => {
    for (const name of allRegistryProviders) {
      const d = resolveDescriptor(name, false);
      expect(d.providerId).not.toBe(OpenClawProviderId.Agora);
    }
  });
