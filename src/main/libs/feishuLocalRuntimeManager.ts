...
const LocalRuntimeLabel = {
  OpenClaw: 'ai.agora.openclaw.feishu',
  Hermes: 'ai.agora.hermes.feishu',
} as const;
...
const buildPaths = (engineKey: FeishuEngineKeyType): LaunchAgentPaths => {
  ...
  const logDir = path.join(os.homedir(), 'Library', 'Logs', 'Agora');
  return {
    label,
...
export const transferFeishuToAgoraRuntime = async (
  engineKey: FeishuEngineKeyType,
): Promise<FeishuRuntimeOwnershipTransferResult> => {
  try {
    if (!isSupportedEngine(engineKey)) {
    ...
    return {
      success: true,
      status: getFeishuRuntimeOwnershipStatus(engineKey, FeishuRuntimeOwnership.AgoraManaged),
    };
  } catch (error) {
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Failed to transfer Feishu to Agora runtime.',
    };
  }
};
