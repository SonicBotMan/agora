...
function tryAgoraServerFallback(modelId?: string): MatchedProvider | null {
  ...
  console.log('[ClaudeSettings] agora-server fallback activated:', { baseURL, modelId: effectiveModelId, supportsImage: cachedMeta?.supportsImage });
  return {
...
function resolveMatchedProvider(
  ...
  if (!modelId) {
    ...
    if (!fallback) {
      const serverFallback = tryAgoraServerFallback(configuredModelId);
  ...
  // Handle agora-server provider: dynamically construct from auth tokens
  if (preferredProviderName === ProviderName.AgoraServer) {
    const serverMatch = tryAgoraServerFallback(modelId);
    if (serverMatch) {
  ...
  if (!providerEntry) {
    ...
    if (fallback) {
      ...
      const serverFallback = tryAgoraServerFallback(modelId);
      if (serverFallback) return { matched: serverFallback };
...
export function resolveCurrentApiConfig(
  ...
  const effectiveApiKey = resolvedApiKey
    || (!providerRequiresApiKey(matched.providerName) ? 'sk-agora-local' : '');
  ...
  return {
    config: {
      apiKey: resolvedApiKey || 'agora-openai-compat',
...
export function resolveAllProviderApiKeys(): Record<string, string> {
  ...
  // agora-server token is now managed by the token proxy
  // (openclawTokenProxy.ts) — no longer injected as an env var.

    // agora-server: uses auth accessToken
    const tokens = authTokensGetter?.();
    const serverBaseUrl = serverBaseUrlGetter?.();
    ...
    for (const [providerName, providerConfig] of Object.entries(appConfig.providers)) {
      ...
      result[envName] = apiKey || 'sk-agora-local';
    }
...
export function resolveAllEnabledProviderConfigs(): ProviderRawConfig[] {
  ...
  for (const [providerName, providerConfig] of Object.entries(appConfig.providers)) {
    if (!providerConfig?.enabled) continue;
    if (providerName === ProviderName.AgoraServer) continue;
