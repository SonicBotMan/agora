...
const AGORA_PROVIDER_MARKER = 'agora';
...
const sanitizeProviderKey = (value: string): string => {
  const key = value
    ...
    .replace(/[^a-z0-9_]/g, '_')
    .replace(/^_+|_+$/g, '');
  return key || AGORA_PROVIDER_MARKER;
};

const providerDisplayName = (value: string | undefined): string => {
  const normalized = value?.trim() || AGORA_PROVIDER_MARKER;
...
export const mergeOpenCodeConfigForAgoraModel = (
  existingConfig: OpenCodeConfig,
  config: CoworkApiConfig,
  providerName?: string,
): OpenCodeConfig => {
  const isAnthropic = config.apiType === 'anthropic';
  const providerKey = isAnthropic ? 'anthropic' : sanitizeProviderKey(providerName || 'agora');
  const displayName = isAnthropic ? 'Anthropic' : providerDisplayName(providerName || 'agora');
...
export const buildOpenCodeRuntimeConfigContent = (
  ...
  return JSON.stringify(mergeOpenCodeConfigForAgoraModel({}, config, providerName));
};
