...
export const HERMES_AGORA_MODEL_ENV_BLOCK = 'agora-model';
export const HERMES_AGORA_FEISHU_ENV_BLOCK = 'agora-feishu';
...
const managedDotenvStart = (blockId: string): string => `# >>> Agora managed: ${blockId}`;
const managedDotenvEnd = (blockId: string): string => `# <<< Agora managed: ${blockId}`;
...
export const buildHermesFeishuEnvForInstances = (
  ...
  if (enabledInstances.length > 1) {
    return {
      env: {},
      error: 'Hermes Agent currently supports one enabled Feishu bot in Agora. Disable the extra Feishu instances before starting Hermes.',
    };
  }
...
export const buildHermesEnvForAgoraModel = (
...
export const mergeHermesConfigForAgoraModel = (
