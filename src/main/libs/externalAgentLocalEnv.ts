...
const INTERNAL_PROVIDER_META_KEY = '__agoraProviderMeta';
...
const pickCredentialForPrintMode = (
  ...
  const agoraConfig = resolveRawApiConfig().config;
  const apiKeyLooksInjected = Boolean(
    agoraConfig?.apiKey
    && apiKey === agoraConfig.apiKey
    && !baseUrlsMatch(localBaseUrl, agoraConfig.baseURL),
  );
  const authTokenLooksInjected = Boolean(
    agoraConfig?.apiKey
    && authToken === agoraConfig.apiKey
    && !baseUrlsMatch(localBaseUrl, agoraConfig.baseURL),
  );
