...
export class OpenClawEngineManager extends EventEmitter {
  ...
  private async doStartGateway(): Promise<OpenClawEngineStatus> {
    ...
    if (this.requireManagedGateway && !isGatewayProcessAlive(this.gatewayProcess) && await isPortReachable('127.0.0.1', port)) {
      this.setStatus({
        ...
        version: runtime.version,
        progressPercent: 5,
        message: 'Stopping local OpenClaw gateway service before starting Agora-managed gateway...',
        canRetry: false,
        gatewayMode: 'managed',
        gatewayPort: port,
      });
  ...
  private async buildGatewayEnv(
    ...
    const env: NodeJS.ProcessEnv = {
      ...
      OPENCLAW_LOG_LEVEL: process.env.AGORA_OPENCLAW_LOG_LEVEL || process.env.OPENCLAW_LOG_LEVEL || 'warn',
      ...this.secretEnvVars,
    };
