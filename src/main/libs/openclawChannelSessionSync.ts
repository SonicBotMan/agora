 ...
 * to local Cowork sessions so that conversations are visible in the Agora UI.
 */
...
const MANAGED_SESSION_NAMESPACE = 'agora';
const LEGACY_MANAGED_SESSION_NAMESPACE = 'agora';
const AGORA_SESSION_PREFIX = `${MANAGED_SESSION_NAMESPACE}:`;
...
export function parseManagedSessionKey(sessionKey: string | undefined | null): ManagedSessionKey | null {
  ...
  if (raw.startsWith(AGORA_SESSION_PREFIX)) {
    const sessionId = raw.slice(AGORA_SESSION_PREFIX.length).trim();
...
export class OpenClawChannelSessionSync {
  ...
  resolveOrCreateSession(sessionKey: string): string | null {
    // 1. Skip Agora-originated sessions
    if (isManagedSessionKey(sessionKey)) {
      console.debug('[ChannelSessionSync] skipped Agora-originated session');
      return null;
    }
