...
const providerApiKeyEnvVar = (providerName: string): string => {
  ...
  return `AGORA_APIKEY_${envName}`;
};

const API_KEY_PLACEHOLDER_PATTERN = /\$\{((?:WESIGHT|LOBSTER)_APIKEY_[A-Z0-9_]+)\}/g;
const MANAGED_AGENTS_MARKER = '<!-- Agora managed: do not edit below this line -->';
...
const MANAGED_WEB_SEARCH_POLICY_PROMPT = [
  ...
  '- Only use the Agora `web-search` skill when local command execution is available. Native channel sessions may deny `exec`, so prefer `browser` or `web_fetch` there.',
  ...
  '',
  'Do not claim you searched the web unless you actually used `browser`, `web_fetch`, or the Agora `web-search` skill.',
].join('\n');
...
const PROVIDER_REGISTRY: Record<string, ProviderDescriptor> = {
  [ProviderName.AgoraServer]: {
    providerId: OpenClawProviderId.AgoraServer,
  ...
  [ProviderName.Copilot]: {
    providerId: OpenClawProviderId.AgoraCopilot,
    resolveApi: () => OpenClawApiConst.OpenAICompletions as OpenClawProviderApi,
    normalizeBaseUrl: stripChatCompletionsSuffix,
...
const DEFAULT_DESCRIPTOR: ProviderDescriptor = {
  providerId: OpenClawProviderId.Agora,
  resolveApi: ({ apiType, baseURL }) => mapApiTypeToOpenClawApi(apiType, undefined, baseURL),
  normalizeBaseUrl: stripChatCompletionsSuffix,
};
...
const resolveDescriptor = (
  ...
  return {
    ...DEFAULT_DESCRIPTOR,
    providerId: providerName || OpenClawProviderId.Agora,
  };
};
...
export class OpenClawConfigSync {
  ...
  sync(reason: string): OpenClawConfigSyncResult {
    ...
    const proxyPort = getOpenClawTokenProxyPort();
    if (proxyPort && !allProvidersMap[ProviderName.AgoraServer]) {
      ...
      const firstServerSel = buildProviderSelection({
        ...
        apiType: 'openai',
        providerName: ProviderName.AgoraServer,
        supportsImage: serverModels[0]?.supportsImage,
      });
      const agoraProviderConfig = { ...firstServerSel.providerConfig, models: [] as typeof firstServerSel.providerConfig.models };
      for (const sm of serverModels) {
        agoraProviderConfig.models.push({
          id: sm.modelId,
          name: sm.modelId,
      ...
      if (agoraProviderConfig.models.length === 0) {
        agoraProviderConfig.models.push(firstServerSel.providerConfig.models[0]);
      }
      allProvidersMap[OpenClawProviderId.AgoraServer] = agoraProviderConfig;
    }
  ...
  collectSecretEnvVars(): Record<string, string> {
    ...
    for (const [envSuffix, apiKey] of Object.entries(allApiKeys)) {
      env[`AGORA_APIKEY_${envSuffix}`] = apiKey;
    ...
    for (const provider of resolveAllEnabledProviderConfigs()) {
      ...
      const envName = `AGORA_APIKEY_${envSuffix}`;
      if (!env[envName]) {
        env[envName] = provider.apiKey?.trim() || 'sk-agora-local';
      }
  ...
  private syncManagedSessionStore(selection: OpenClawProviderSelection): boolean {
    ...
    for (const [sessionKey, rawEntry] of Object.entries(sessionStore)) {
      ...
      if (!shouldMigrateManagedModelRefs || !(/^agent:[^:]+:(?:agora|agora):/.test(sessionKey))) {
        continue;
      }
  ...
  /**
   * Resolve the Agora SKILLs installation directory for OpenClaw's
  ...
  /**
   * Build per-skill `enabled` overrides from the Agora SkillManager state,
   * so that skills disabled in the Agora UI are also hidden from OpenClaw.
   */
   ...
   * native runtime can discover and invoke Agora skills.
   */
