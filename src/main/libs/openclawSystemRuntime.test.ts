...
describe('openclawSystemRuntime', () => {
  ...
  it('imports direct local Feishu config as a disabled Agora draft', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'agora-openclaw-feishu-'));
  ...
  it('detects env placeholder secrets that need user input', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'agora-openclaw-feishu-env-'));
    const configPath = path.join(dir, 'openclaw.json');
    fs.writeFileSync(configPath, JSON.stringify({
      channels: {
        'openclaw-feishu': {
