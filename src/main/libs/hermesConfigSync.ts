...
export class HermesConfigSync {
  ...
  constructor(deps: HermesConfigSyncDeps) {
    ...
    this.getFeishuRuntimeOwnership = deps.getFeishuRuntimeOwnership ?? (() => FeishuRuntimeOwnership.AgoraManaged);
  }
    ...
    try {
      ...
      if (!apiResolution.config) {
        if (coworkConfig.agentEngine === CoworkAgentEngine.Hermes) {
          throw new Error(apiResolution.error || 'No Agora model is configured.');
        }
      }
      ...
      if (apiResolution.config) {
        ...
        const existingConfig = parseHermesConfigText(readText(this.engineManager.getConfigPath()));
        const nextConfig = mergeHermesConfigForAgoraModel(existingConfig, apiResolution.config, {
        ...
        nextEnvText = mergeHermesManagedDotenvBlock(
          nextEnvText,
          HERMES_AGORA_MODEL_ENV_BLOCK,
          buildHermesEnvForAgoraModel(apiResolution.config),
        );
      }
