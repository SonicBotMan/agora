...
import {
  ...
  mergeDeepSeekTuiConfigForAgoraModel,
...
const requireApiConfig = (resolution: ReturnType<typeof resolveRawApiConfig>): CoworkApiConfig => {
  if (!resolution.config) {
    throw new Error(resolution.error || 'No Agora model is configured.');
  }
...
const buildAgoraCcSwitchProviderName = (providerName: string | undefined): string => {
  return `Agora - ${formatProviderDisplayName(providerName)}`;
};
...
const findCcSwitchProviderForConfig = (
  ...
  const agoraProviderName = normalizeProviderName(buildAgoraCcSwitchProviderName(providerName));
  const existingAgoraProvider = providers.find((provider) => {
    return normalizeProviderName(provider.name) === agoraProviderName
      || getString(provider.meta.managedBy) === 'agora';
  });
  if (existingAgoraProvider) {
    return existingAgoraProvider;
  }
...
const upsertCcSwitchClaudeProvider = (
  ...
  const meta = {
    ...
    apiFormat: 'anthropic',
    ...(targetProvider ? {} : { managedBy: 'agora' }),
  };
  ...
  if (targetProvider) {
    ...
    db
      ...
      .run(
        providerId,
        'claude',
        buildAgoraCcSwitchProviderName(providerName),
        JSON.stringify(settingsConfig),
        'agora',
        now,
        maxSortIndex + 1,
...
const syncClaudeCodeFromAgoraModel = (): void => {
...
const syncCodexFromAgoraModel = (): void => {
  const resolved = resolveRawApiConfig();
  const config = requireApiConfig(resolved);
  if (config.apiType === 'anthropic') {
    throw new Error('Codex 引擎跟随 Agora 模型设置时，需要选择 OpenAI 兼容的模型配置。');
  }

  const providerName = resolved.providerMetadata?.providerName || 'agora';
...
export const syncOpenCodeGlobalConfigFromAgoraModel = (): void => {
  const resolved = resolveRawApiConfig();
  const config = requireApiConfig(resolved);
  ...
  writeJsonObject(
    paths.primaryConfigPath,
    mergeOpenCodeConfigForAgoraModel(existing, config, resolved.providerMetadata?.providerName),
  );
};
...
export const syncDeepSeekTuiGlobalConfigFromAgoraModel = (): void => {
  ...
  atomicWrite(
    paths.primaryConfigPath,
    serializeDeepSeekTuiConfig(
      mergeDeepSeekTuiConfigForAgoraModel(existing, config, resolved.providerMetadata?.providerName),
    ),
  );
};
...
export const applyExternalAgentConfigForEngine = (
  ...
  if (engine === CoworkAgentEngine.ClaudeCode) {
    syncClaudeCodeFromAgoraModel();
    return;
  }
  if (engine === CoworkAgentEngine.Codex) {
    syncCodexFromAgoraModel();
    return;
  }
...
const readCodexLocalConfig = (): { apiKey: string; baseUrl: string; model: string } => {
  ...
  if (!apiKey) {
    throw new Error('本机 Codex 配置看起来使用登录态，无法直接导入为 Agora API 模型配置。可继续使用“本机 CLI 配置”模式。');
  }
  if (!baseUrl) {
