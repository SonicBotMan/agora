...
describe('qwenCodeConfig', () => {
  ...
  test('merges Agora model while preserving unknown fields', () => {
    const merged = mergeQwenCodeConfigForAgoraModel(
      {
        ui: { theme: 'dark' },
        env: {
          OTHER_KEY: 'keep',
    ...
    expect(merged.env?.AGORA_QWEN_OPENAI_API_KEY).toBe('sk-new');
  ...
  test('summarizes stored Qwen settings', () => {
    const summary = summarizeQwenCodeSettingsConfig({
      ...
      config: {
        env: {
          AGORA_QWEN_OPENAI_API_KEY: 'sk-qwen',
        },
        modelProviders: {
          openai: [
            {
              id: 'qwen3-coder-plus',
              envKey: 'AGORA_QWEN_OPENAI_API_KEY',
