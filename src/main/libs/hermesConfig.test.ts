...
describe('hermesConfig', () => {
  ...
  test('merges Agora model while preserving unknown fields', () => {
    const merged = mergeHermesConfigForAgoraModel(
      {
  ...
  test('maps Anthropic model into Hermes config and env', () => {
    ...
    const env = buildHermesEnvForAgoraModel({
      apiKey: 'sk-ant',
      baseURL: 'https://api.anthropic.com',
      model: 'claude-sonnet-4-5',
      apiType: 'anthropic',
  ...
  test('maps BigModel Anthropic-compatible endpoint to Z.AI provider', () => {
    ...
    const merged = mergeHermesConfigForAgoraModel({}, config);
    const env = buildHermesEnvForAgoraModel(config);
  ...
  test('merges managed dotenv block while preserving user values', () => {
    ...
    expect(merged).toContain('# >>> Agora managed: agora-feishu');
