...
export class OpenClawRuntimeAdapter extends EventEmitter implements CoworkRuntime {
   ...
   * so Agora can recover even when the gateway fails to deliver the abort event.
   */
  ...
  private buildSystemPromptPrefix(systemPrompt: string): string {
    return [
      '[Agora system instructions]',
      'Apply the instructions below as the highest-priority guidance for this session.',
      'If earlier Agora system instructions exist, replace them with this version.',
      systemPrompt,
    ].join('\n');
  }
  ...
  private buildBridgePrefix(messages: CoworkMessage[], currentPrompt: string): string {
    ...
    return [
      '[Context bridge from previous Agora conversation]',
  ...
  private async approveGatewayPairingIfNeeded(
    ...
    console.log('[OpenClawRuntime] approved local gateway pairing request for Agora');
    return true;
  }
   ...
   * gateway rather than the Agora UI.
   *
