...
export class ExternalCliRuntimeAdapter extends EventEmitter implements CoworkRuntime {
    ...
    if (this.engine === CoworkAgentEngine.ClaudeCode) {
      if (claudeCodePermissionMode === ClaudeCodePermissionMode.Plan) {
        runtimeNoteLines.push(
          '- Agora runs Claude Code in plan mode. Present the plan clearly and wait for the CLI plan flow.',
        );
      } else {
        runtimeNoteLines.push(
          '- Agora runs Claude Code as a graphical task executor. Do not enter planning-only flows or wait for plan approval.',
  ...
  private materializeImageAttachments(
    ...
    const dir = path.join(os.tmpdir(), 'agora-cli-images', sessionId);
  ...
  private resolveCodexGeneratedImagePath(
    ...
    const targetPath = defaultPath ?? path.join(os.tmpdir(), 'agora-codex-images', active.sessionId, `${imageId}.png`);
    try {
