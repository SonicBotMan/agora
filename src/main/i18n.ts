 ...
 *   setLanguage('en');
 *   const label = t('trayShowWindow'); // "Open Agora"
...
export const MAIN_I18N_TRANSLATIONS: TranslationCatalog = {
  zh: {
    ...
    imFeishuHermesHint: '飞书将通过 Hermes Agent 运行，Agora 已同步当前启用实例到 Hermes 配置。',
    imFeishuClaudeCodeHint: '飞书将由 Agora 原生长连接接入，并通过本机 Claude Code CLI 执行任务。',
    ...
    imFeishuCodexHint: '飞书将由 Agora 原生长连接接入，并通过本机 Codex CLI 执行任务。',
    ...
    imFeishuLocalOpenClawOwnerSuggestion: '当前飞书配置归本机 OpenClaw 管理。请先在飞书设置中切换为 Agora 统一管理，再启动当前引擎的飞书链路。',
  ...
  en: {
    ...
    imFeishuHermesHint: 'Feishu will run through Hermes Agent. Agora has synced the enabled instance into Hermes config.',
    imFeishuClaudeCodeHint: 'Feishu will use the Agora native long connection and execute tasks through the local Claude Code CLI.',
    ...
    imFeishuCodexHint: 'Feishu will use the Agora native long connection and execute tasks through the local Codex CLI.',
    ...
    imFeishuLocalOpenClawOwnerSuggestion: 'Feishu is currently managed by local OpenClaw. Switch to Agora management in Feishu settings before starting this engine link.',
