...
export const normalizeApiFormat = (
  value: unknown,
): 'anthropic' | 'openai' => (
...
export const shouldAutoSwitchProviderBaseUrl = (
  ...
  const anthropicUrl = ProviderRegistry.getSwitchableBaseUrl(
    provider,
    'anthropic',
  );
  ...
  return (
    (anthropicUrl
      ? normalizedCurrent === normalizeBaseUrl(anthropicUrl)
      : false)
...
export const resolveBaseUrl = (
  ...
  if (baseUrl.trim()) {
    if (
      ...
      const switchedUrl = ProviderRegistry.getSwitchableBaseUrl(
        provider,
        apiFormat,
      );
...
export const shouldUseOpenAIResponsesForProvider = (
  provider: string,
...
export const getDefaultActiveProvider = (): ProviderType => {
  ...
  const defaultModelProvider = defaultConfig.model
    .defaultModelProvider as ProviderType | undefined;
  return (
    firstEnabledProvider
    ?? (
      defaultModelProvider && providers[defaultModelProvider]
...
export const getCustomProviderLabel = (provider: ProviderType): string => (
  getCustomProviderDefaultName(provider)
);
