...
const RuntimeDashboardView: React.FC<RuntimeDashboardViewProps> = ({
  ...
  useEffect(() => {
    ...
    window.addEventListener('agora-agent-provider-changed', handleProviderChanged);
    return () => {
      window.removeEventListener('agora-agent-provider-changed', handleProviderChanged);
    };
  }, [localCliAppType]);
