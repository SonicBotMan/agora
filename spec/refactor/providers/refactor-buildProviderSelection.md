...
| Provider | 处理方式 | 状态 |
|----------|----------|------|
| `agora-server` | 专用分支（token proxy） | ✅ 已覆盖 |
...
| 类别 | 散落的字符串 | 出现位置 |
|------|-------------|---------|
| Provider Name | `'agora-server'`, `'moonshot'`, `'gemini'` 等 | `buildProviderSelection` 的 if 条件、`claudeSettings.ts` 的 `getEffectiveProviderApiFormat`、renderer `config.ts` 的 key |
...
type ProviderDescriptor = {
   ...
   * Most providers use env var placeholders; agora-server may use 'proxy-managed'.
...
### Step 3: 保留 `agora-server` 的日志

原函数中 `agora-server` 分支有 `console.log` 输出调试信息（搜索 `buildProviderSelection agora-server` 定位），需在 descriptor 的 `normalizeBaseUrl` 或核心函数中保留。

**QA**（通过 Step 4 的测试验证，运行 `npm test -- openclawConfigSync`）:
1. 测试用例 `'agora-server logs proxy info'`：使用 `vi.spyOn(console, 'log')` 断言当 `providerName === 'agora-server'` 时，`console.log` 被调用且参数包含 `'[OpenClawConfigSync]'` 标签。
...
```typescript
// 场景 1: agora-server（token proxy 模式）
// 输入: providerName='agora-server', baseURL='http://example.com/v1/chat/completions', apiType='openai'
// 断言:
//   providerId === 'agora-server'
